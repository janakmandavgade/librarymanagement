...
def addbook_view(request):
    ...
    if request.method == 'POST':
        ...
        form = forms.BookForm(request.POST)  # {id_i}
        if form.is_valid():
            user = form.save()
            print(user.name)
...
def editBook(request, id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            original_book = models.Book.objects.get(id=id)

            # original_book.name = book.name
            # original_book.isbn = book.isbn
            # original_book.author = book.author
            # original_book.category = book.category

            original_book.name = request.POST['name']
            original_book.isbn = request.POST['isbn']
            original_book.author = request.POST['author']
            original_book.category = request.POST['category']
